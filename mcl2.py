...
class Particle_filter(object):
    ...
    def velocity_motion_model(self,msg):
        ...
        if  self.last_orientation and self.opt_velocity:
            ...
            self.roll = q_map_lastbaselink_euler[0]
            self.pitch = q_map_lastbaselink_euler[1]
            if self.droll < 0.1:
                self.dy_temp += self.opt_velocity.integrated_x*self.altitude*mt.tan(self.roll)
            if self.dpitch < 0.1:
                self.dx_temp += self.opt_velocity.integrated_y*self.altitude*mt.tan(self.pitch)

            print('roll', self.roll, 'pitch', self.pitch)
            print('droll', self.droll, 'dpitch', self.dpitch)
